...
class WR_EW_Promo_Grid_Full extends Widget_Base {
    ...
    public function get_name() { return 'wr-promo-grid-full'; }
    public function get_title() { return __( 'WR Promo Grid Full', 'wr-ew' ); }
    public function get_icon() { return 'eicon-gallery-grid'; }
    public function get_categories() { return [ 'wr-widgets' ]; }
    public function get_style_depends() { return [ self::STYLE_HANDLE ]; }
    public function get_script_depends() { return [ self::SCRIPT_HANDLE ]; }
    ...
    private function get_youtube_id( $url ) {
        ...
        if ( preg_match( '~/(embed|shorts)/([A-Za-z0-9_-]{6,})~', $url, $m ) ) return $m[2];
    ...
    protected function register_controls() {
        ...
        /**
         * PATCH: Per-tile Content Position overrides
         * (Offset X/Y + Max Width + Padding)
         */
        $repeater->add_control(
            'tile_offset_x',
            [
        ...
        $repeater->add_control(
            'tile_content_max_width',
            [
                'label'      => __( 'Tile Content Max Width', 'wr-ew' ),
                'type'       => Controls_Manager::SLIDER,
                'size_units' => [ 'px' ],
                'range'      => [ 'px' => [ 'min' => 120, 'max' => 900, 'step' => 1 ] ],
                'default'    => [ 'size' => '', 'unit' => 'px' ],
    ...
    private function build_tile_inline_vars( $tile, $settings ) {
        ...
        if ( isset( $tile['tile_offset_x']['size'] ) && $tile['tile_offset_x']['size'] !== '' ) {
            $vars[] = '--wr-pgfull-x:' . (float) $tile['tile_offset_x']['size'] . 'px';
        }
        if ( isset( $tile['tile_offset_y']['size'] ) && $tile['tile_offset_y']['size'] !== '' ) {
            $vars[] = '--wr-pgfull-y:' . (float) $tile['tile_offset_y']['size'] . 'px';
        }

        return implode( ';', $vars );
    }

    private function build_tile_content_inline_style( $tile ) {
        $style = '';

        if ( isset( $tile['tile_content_max_width']['size'] ) && $tile['tile_content_max_width']['size'] !== '' ) {
            $style .= 'max-width:' . (float) $tile['tile_content_max_width']['size'] . 'px;';
        }

        // Tile padding override (if any side provided)
        if ( ! empty( $tile['tile_content_padding'] ) && is_array( $tile['tile_content_padding'] ) ) {
            $p = $tile['tile_content_padding'];

            $has_any =
                ( isset( $p['top'] ) && $p['top'] !== '' ) ||
                ( isset( $p['right'] ) && $p['right'] !== '' ) ||
                ( isset( $p['bottom'] ) && $p['bottom'] !== '' ) ||
                ( isset( $p['left'] ) && $p['left'] !== '' );

            if ( $has_any ) {
                $unit = ! empty( $p['unit'] ) ? $p['unit'] : 'px';
                $top    = ( $p['top']    !== '' ) ? $p['top']    . $unit : '0';
                $right  = ( $p['right']  !== '' ) ? $p['right']  . $unit : '0';
                $bottom = ( $p['bottom'] !== '' ) ? $p['bottom'] . $unit : '0';
                $left   = ( $p['left']   !== '' ) ? $p['left']   . $unit : '0';
                $style .= 'padding:' . esc_attr( $top ) . ' ' . esc_attr( $right ) . ' ' . esc_attr( $bottom ) . ' ' . esc_attr( $left ) . ';';
            }
        }
    ...
    private function render_tile( $tile, $hover_effect, $settings ) {
        ...
        if ( $hover_effect && 'none' !== $hover_effect ) $classes[] = 'wr-pgfull__tile--hover-' . $hover_effect;
        ...
        <<?php echo $tag; ?> class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>"<?php echo $attrs; ?><?php echo $style_attr; ?>>
            ...
            <div class="wr-pgfull__content wr-pgfull__content--pos-<?php echo esc_attr( $pos ); ?>"<?php echo $content_style_attr; ?>>
                ...
                <?php
                // If tile is linked, do NOT render nested anchors.
                // If tile is NOT linked, button may be anchor when URL exists.
                if ( ! $tile_link && ! empty( $tile['button_text'] ) ) :
                    $btn_url = $link_url;
                    if ( $btn_url ) :
                        $btn_target = ( $new_tab || $tile_link_is_external ) ? ' target="_blank"' : '';
                        $btn_rel = [];
                        if ( $tile_link_nofollow ) $btn_rel[] = 'nofollow';
                        if ( $new_tab || $tile_link_is_external ) $btn_rel[] = 'noopener';
