...
class WR_EW_Promo_Grid_Full extends Widget_Base {
    ...
    protected function get_youtube_embed_src( $url ) {
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            ...
            [
                ...
                'options' => [
                    ...
                    'three_equal'        => [ 'title' => __( 'Three Equal', 'wr-ew' ), 'icon' => 'eicon-column' ],
                    'three_mosaic_left'  => [ 'title' => __( 'Mosaic L', 'wr-ew' ), 'icon' => 'eicon-column' ],
                    'three_mosaic_right' => [ 'title' => __( 'Mosaic R', 'wr-ew' ), 'icon' => 'eicon-column' ],
                    'four_checker'       => [ 'title' => __( 'Checker', 'wr-ew' ), 'icon' => 'eicon-column' ],
                    'four_mosaic_left'   => [ 'title' => __( '4 Mosaic L', 'wr-ew' ), 'icon' => 'eicon-column' ],
                    'four_mosaic_right'  => [ 'title' => __( '4 Mosaic R', 'wr-ew' ), 'icon' => 'eicon-column' ],
                    'six_grid'           => [ 'title' => __( 'Six Grid', 'wr-ew' ), 'icon' => 'eicon-column' ],
                ],
                'toggle' => false,
            ]
        );
        ...
        $repeater->add_control(
            'content_position',
            [
                'label'   => __( 'Overlay Position', 'wr-ew' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'bottom-left',
                'options' => [
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .wr-pgfull__media img, {{WRAPPER}} .wr-pgfull__media video' => 'transform: scale({{SIZE}});',
                ],
            ]
        );
        ...
        $this->add_control(
            ...
            [
                'label' => __( 'Button Text Hover', 'wr-ew' ),
                'type'  => Controls_Manager::COLOR,
                'selectors' => [
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'selectors'  => [
                    '{{WRAPPER}} .wr-pgfull__content' => '--wr-pgfull-offset-x: {{SIZE}}{{UNIT}};',
                ],
            ]
        );
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'range'      => [ 'px' => [ 'min' => -120, 'max' => 120 ] ],
                'selectors'  => [
                    '{{WRAPPER}} .wr-pgfull__content' => '--wr-pgfull-offset-y: {{SIZE}}{{UNIT}};',
                ],
            ]
        );
    ...
    protected function render_tile( $tile, $index, $hover_effect ) {
        if ( empty( $tile['show_tile'] ) || 'yes' !== $tile['show_tile'] ) {
            return '';
        }

        $tile_type = isset( $tile['tile_type'] ) ? $tile['tile_type'] : 'image';
        $link      = isset( $tile['link']['url'] ) ? $tile['link']['url'] : '';
        $is_new    = ! empty( $tile['new_tab'] ) && 'yes' === $tile['new_tab'];
        $overlay   = empty( $tile['overlay_enable'] ) || 'yes' === $tile['overlay_enable'];
        $position  = ! empty( $tile['content_position'] ) ? $tile['content_position'] : 'bottom-left';

        $tag        = $link ? 'a' : 'div';
        $attr_link  = $link ? ' href="' . esc_url( $link ) . '"' : '';
        $attr_target = $link && $is_new ? ' target="_blank" rel="noopener"' : '';

        $classes = [
            'wr-pgfull__tile',
            'wr-pgfull__tile--' . $tile_type,
        ];
        ...
        ob_start();
        ?>
        <<?php echo $tag; ?> class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>"<?php echo $attr_link; ?><?php echo $attr_target; ?>>
            <div class="wr-pgfull__media">
                    ...
                    <?php if ( $this->is_youtube_url( $tile['video_url'] ) ) : ?>
                        ...
                        <?php if ( $yt_src ) : ?>
                            <iframe
                                ...
                                title="<?php echo esc_attr( $tile['title'] ?? 'YouTube video' ); ?>"
                ...
                <?php elseif ( ! empty( $tile['image']['url'] ) ) : ?>
                    <img src="<?php echo esc_url( $tile['image']['url'] ); ?>" alt="<?php echo esc_attr( $tile['title'] ?? '' ); ?>" loading="lazy" />
                <?php else : ?>
                    <div class="wr-pgfull__placeholder"></div>
                <?php endif; ?>
            </div>
            ...
            <div class="wr-pgfull__content wr-pgfull__content--pos-<?php echo esc_attr( $position ); ?>">
                ...
                <?php if ( empty( $link ) && ! empty( $tile['button_text'] ) && ! empty( $tile['link']['url'] ) ) : ?>
                    <a class="wr-pgfull__button" href="<?php echo esc_url( $tile['link']['url'] ); ?>"<?php echo $is_new ? ' target="_blank" rel="noopener"' : ''; ?>><?php echo esc_html( $tile['button_text'] ); ?></a>
                <?php elseif ( empty( $link ) && ! empty( $tile['button_text'] ) ) : ?>
    ...
    protected function render() {
        ...
        $max_tiles = $max_tiles_per_preset[ $preset ] ?? 6;
        $tiles_raw = isset( $settings['tiles'] ) && is_array( $settings['tiles'] ) ? $settings['tiles'] : [];
        $tiles     = array_slice( $tiles_raw, 0, $max_tiles );
        ...
        echo '<div class="' . esc_attr( implode( ' ', $wrapper_classes ) ) . '"><div class="wr-pgfull__inner">';
        echo '<div class="' . esc_attr( implode( ' ', $grid_classes ) ) . '">';

        foreach ( $tiles as $index => $tile ) {
            echo $this->render_tile( $tile, $index, $hover );
        }
